...
if [ -f /usr/local/etc/ai-gemini-feature.conf ]; then
  source /usr/local/etc/ai-gemini-feature.conf
  echo "✓ Configuration loaded from /usr/local/etc/ai-gemini-feature.conf"
else
  echo "⚠ Configuration file not found, using defaults"
fi

# Skills directory for Gemini CLI (user scope)
echo ""
echo "Setting up skills directory..."
SKILLS_DIR="${USER_HOME}/.gemini/skills"
echo "Skills directory: ${SKILLS_DIR}"
mkdir -p "${USER_HOME}/.gemini" "${SKILLS_DIR}"
...
if [ ${INSTALLED_COUNT} -gt 0 ]; then
  echo "✓ Gemini CLI skills are ready to use"
fi
